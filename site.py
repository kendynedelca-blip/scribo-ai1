...
def create_or_get_oauth_user(provider, provider_data):
    ...
    # Dacă emailul există deja, legăm contul OAuth
    if email:
        existing_email = conn.execute(
            """
            SELECT username
...
if not st.session_state.authenticated:
    ...
    login_logo_cols = st.columns([1, 1, 1])
    ...
    with col_center:
        ...
        st.markdown("### Intră rapid")

        oauth1, oauth2 = st.columns(2)
        ...
        with oauth1:
            ...
            if google_available:
                ...
                st.button(
                    ...
                    disabled=True,
                    use_container_width=True,
                    help="Configurează [auth.google] în secrets.toml",
                )
        ...
        with oauth2:
            ...
            if apple_available:
                ...
                st.button(
                    ...
                    disabled=True,
                    use_container_width=True,
                    help="Configurează [auth.apple] în secrets.toml",
                )
...
with composer:
    ...
    composer_cols = st.columns([1, 1, 8])

    prompt = composer_cols[2].chat_input(
        "Întreabă orice"
    )
